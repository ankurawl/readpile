...
class TestBasicProcessing:
    ...
    async def test_returns_entries(self, state_dir, wiki_dir):
        ...
        with patch(
            ...
            processor = FeedProcessor([source], state, {}, registry=None)
...
class TestFirstSyncEntryCap:
    ...
    async def test_first_sync_caps_at_20(self, state_dir, wiki_dir):
        ...
        with patch(
            ...
            processor = FeedProcessor([source], state, config, registry=None)
            results = await processor.check_all()

        assert len(results) == 20
        # The remaining 80 should still be marked as seen
        seen = state.get_feed_seen_urls(source.url)
        assert len(seen) == 100


class TestFeedRedirectDetection:
    """When feed_info href differs from source URL, update_url is called."""
    ...
    async def test_redirect_updates_url(self, state_dir, wiki_dir, tmp_path):
        ...
        with patch(
            ...
            processor = FeedProcessor(
                [source], state, {}, registry=mock_registry,
            )
            await processor.check_all()

        mock_registry.update_url.assert_called_once_with(source.name, new_url)


class TestConsecutiveFailureTracking:
    """After max_consecutive_failures, the source is auto-disabled."""

    @pytest.mark.asyncio
    async def test_seven_failures_disables_source(self, state_dir, wiki_dir):
        ...
        with patch(
            ...
            processor = FeedProcessor(
                [source], state, config, registry=mock_registry,
            )
            results = await processor.check_all()

        assert len(results) == 0
        mock_registry.disable.assert_called_once_with(source.name)
        assert state.get_consecutive_failures(source.url) == 7


class TestDisabledSourceSkipped:
    """A disabled source is not crawled at all."""

    @pytest.mark.asyncio
    async def test_disabled_source_skipped(self, state_dir, wiki_dir):
        _write_state(state_dir)
        source = _make_source(disabled=True)
...
class TestYouTubeSourceResolvesURL:
    """YouTube sources call resolve_youtube_feed."""

    @pytest.mark.asyncio
    async def test_youtube_source_resolves(self, state_dir, wiki_dir):
        _write_state(state_dir)
        source = _make_source(
            name="YT Channel",
        ...
        with (
            ...
            processor = FeedProcessor([source], state, {}, registry=None)
            results = await processor.check_source(source)
...
class TestSuccessResetsFailures:
    ...
    async def test_success_resets_failures(self, state_dir, wiki_dir):
        ...
        assert state.get_consecutive_failures(source.url) == 0
