"""Configuration — load and validate mediakit settings."""
...
def get_config_path() -> Path:
    """Return the path to the mediakit config file.

    Respects the ``MEDIAKIT_CONFIG`` environment variable; falls back to
    ``~/.mediakit/config.toml``.
    """
    env = os.environ.get("MEDIAKIT_CONFIG")
    if env:
        return Path(env).expanduser()
    return Path("~/.mediakit/config.toml").expanduser()
